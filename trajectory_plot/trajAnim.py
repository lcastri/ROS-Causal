...
# Params definition
AGENT= 'A1'
ANIM_DIR = os.getcwd() + '/trajectory_plot/'
...
# Define goals
with open(GOALS_PATH) as json_file:
...
if not GIF:
    ...
    if not os.path.exists(ANIM_DIR + 'gif/frames'):
        os.makedirs(ANIM_DIR + f'gif/frames')

    # Save frames as images
    for i in range(0, len(trajectory_data), 5):
        update_plot(i)
        plt.savefig(ANIM_DIR + f'gif/frames/frame_{i:04d}.png')

    # Convert frames to GIF using Pillow
    images = []
    for i in range(0, len(trajectory_data), 5):
        images.append(Image.open(ANIM_DIR + f'gif/frames/frame_{i:04d}.png'))
    images[0].save(ANIM_DIR + 'gif/' + AGENT + '.gif', save_all = True, append_images=images[1:], duration=100, loop=0)

    # Remove frames directory
    frames_dir = ANIM_DIR + 'gif/frames'
